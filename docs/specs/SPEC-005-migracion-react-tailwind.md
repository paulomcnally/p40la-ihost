...
| 2026-08-13 | p40la-ihost-team | Release: v0.2.0 publicada en Docker Hub. Commit `8ad531c`, tag `v0.2.0`. Imagen ARM64 multi-stage (node:20-alpine → golang:1.23-alpine → distroless). |
| 2026-08-13 | p40la-ihost-team | Release: v0.2.1 publicada en Docker Hub para iHost. Commit `e5c5373`, tag `v0.2.1`. Imagen **multi-arch con 3 plataformas** (`linux/amd64` + `linux/arm/v7` + `linux/arm64`) compilada con `docker buildx build --platform linux/amd64,linux/arm/v7,linux/arm64 --push`. Regla documentada en `AGENTS.md`. |
