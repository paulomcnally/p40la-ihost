...
| 2026-09-24 | opencode | **Release**: merge a main, versión 0.4.40, issue #95 cerrado. Feature: sincronización manual de servicios con automation (endpoint `POST /api/services/{id}/sync`, UI, bot `/sincronizar_servicio <id>`, badge `ID: N`). Coordinado con SPEC-017 del repo `p40la-ihost-automation`. |