...
export default function PayBillModal({ bill, onClose, onSuccess }: PayBillModalProps) {
  ...
  const handleConfirm = async () => {
    if (!paidAt) {
      showToast(t('bills.pay_date_required'), 'error')
      return
    }
    setSaving(true)
    try {
      await api.bills.pay(bill.id, {
        paid_at: paidAt,
        drive_url: driveUrl.trim() || undefined,
        payment_reference: paymentReference.trim() || undefined,
      })
      showToast(t('bills.pay_success'), 'success')
      onSuccess()
      onClose()
    } catch (err: unknown) {
    ...
    >
      ...
      >
        <div className="p-5">
          ...
          <h3 className="text-lg sm:text-xl font-semibold mb-1 text-center">{t('bills.pay')}</h3>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">{t('bills.pay_date')}</label>
              <input
                type="date"
                value={paidAt}
                max={todayISO()}
            ...
            <div>
              <label className="block text-sm font-medium mb-1">{t('bills.receipt')}</label>
              <input
                type="url"
                value={driveUrl}
              ...
              />
              <p className="text-xs text-text-secondary mt-1">{t('bills.receipt_hint')}</p>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">{t('bills.payment_reference')}</label>
              <input
                type="text"
                value={paymentReference}
                onChange={(e) => setPaymentReference(e.target.value)}
                className="w-full px-3 py-2 border border-border rounded-ios-sm focus:outline-none focus:border-primary min-h-[44px] bg-card"
                placeholder={t('bills.payment_reference_placeholder')}
                autoComplete="off"
              />
            </div>
          </div>
        ...
        <div className="flex gap-3 p-4 border-t border-border">
          ...
          >
            ...
            {saving ? t('app.loading') : t('bills.pay')}
          </button>
        </div>
      </div>
    </div>
