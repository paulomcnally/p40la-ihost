...
// UpdateAutoService actualiza los datos de póliza de un seguro de un auto.
func (h *AutoServiceHandlers) UpdateAutoService(w http.ResponseWriter, r *http.Request) {
	autoID, err := strconv.ParseInt(r.PathValue("id"), 10, 64)
	if err != nil {
		respondError(w, http.StatusBadRequest, "invalid_id", "ID inválido")
		return
	...
	if err := h.service.Update(r.Context(), autoID, serviceID, req.CoverageType, req.PolicyNumber, req.Certificate, req.InsurerNumber); err != nil {
		if err.Error() == "el seguro no existe" {
	...
	respondJSON(w, http.StatusOK, map[string]string{"message": "Seguro actualizado"})
}
