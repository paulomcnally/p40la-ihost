...
func TestBuildPensionRecordsCreatedEmail(t *testing.T) {
	...
	title, content := buildPensionRecordsCreatedEmail(salaryPayments, records, 2026, 8, DefaultCurrencyFormat())
...
func TestBuildPensionRecordPaidEmail(t *testing.T) {
	...
	title, content := buildPensionRecordPaidEmail(record, DefaultCurrencyFormat())
...
func TestBuildPensionSalaryReceivedEmail(t *testing.T) {
	...
	title, content := buildPensionSalaryReceivedEmail(payment, DefaultCurrencyFormat())
	if !strings.Contains(title, "Salario recibido") {
		t.Fatalf("título incorrecto: %s", title)
	}
	if !strings.Contains(content, "Empresa XYZ") || !strings.Contains(content, "14,500.00") {
		t.Fatalf("contenido incorrecto: %s", content)
	}
}
...
func TestBuildPensionRecordRejectedEmail(t *testing.T) {
	...
	title, content := buildPensionRecordRejectedEmail(record, "Comprobante inválido", DefaultCurrencyFormat())
...
func TestBuildPensionMonthClosingEmail(t *testing.T) {
	...
	title, content := buildPensionMonthClosingEmail(records, salaryPayments, 2026, 8, DefaultCurrencyFormat())
