...
const serviceColumns = `
	...
	last_webhook_request, automation_account_id,
...
func (s *ServiceStorage) Create(ctx context.Context, svc *models.Service) (*models.Service, error) {
	result, err := s.db.ExecContext(ctx, `
		INSERT INTO services (home_id, name, institution, currency_id, frequency, suggested_amount, active, icon_key, billing_type, billing_day, auto_generate, institution_id, institution_analyzer_id, start_date, end_date, is_recurring, webhook_uuid, automation_account_id)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, svc.HomeID, svc.Name, svc.Institution, svc.CurrencyID, svc.Frequency, svc.SuggestedAmount, svc.Active, svc.IconKey, svc.BillingType, svc.BillingDay, svc.AutoGenerate, svc.InstitutionID, svc.InstitutionAnalyzerID, svc.StartDate, svc.EndDate, svc.IsRecurring, svc.WebhookUUID, svc.AutomationAccountID)
	if err != nil {
...
func (s *ServiceStorage) Update(ctx context.Context, svc *models.Service) (*models.Service, error) {
	_, err := s.db.ExecContext(ctx, `
		...
		SET home_id = ?, name = ?, institution = ?, currency_id = ?, frequency = ?,
		    ...
		    webhook_uuid = ?, automation_account_id = ?,
	...
	`, svc.HomeID, svc.Name, svc.Institution, svc.CurrencyID, svc.Frequency,
		...
		svc.InstitutionID, svc.InstitutionAnalyzerID, svc.StartDate, svc.EndDate, svc.IsRecurring,
		svc.WebhookUUID, svc.AutomationAccountID, svc.ID)
	if err != nil {
...
func scanService(row *sql.Row) (*models.Service, error) {
	...
	var automationAccountID sql.NullInt64
	if err := row.Scan(&svc.ID, &svc.HomeID, &svc.Name, &institution, &svc.CurrencyID,
		...
		&lastWebhookRequest, &automationAccountID, &latestBillStatus, &svc.IsInsurance, &deletedAt, &svc.CreatedAt, &svc.UpdatedAt); err != nil {
	...
	if automationAccountID.Valid {
		svc.AutomationAccountID = &automationAccountID.Int64
	}
	return &svc, nil
}
